...
class GetNetworkResult @Inject constructor() {
    private val ioDispatcher: CoroutineDispatcher = Dispatchers.IO


    suspend fun <ResultType> getApiResult(api: suspend () -> Response<ResultType>): Flow<SealedResult<ResultType>> {
        return withContext(ioDispatcher) {
            flow {
                ...
                if (response.isSuccessful) {
                    response.body()?.let {
                        emit(SealedResult.Success(data = it))
                    } ?: emit(SealedResult.Error(message = "Unknown error occurred", code = 0))
                } else {
                    emit(
                        SealedResult.Error(
                ...
                emit(SealedResult.Error(message = message(error), code = code(error)))
            }
        }
    }
